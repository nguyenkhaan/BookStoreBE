...
export class BookController 
{
    ...
    async createBook(@Body() createBookData : CreateBookData , @UploadedFile() file : Express.Multer.File) 
    {
        const responseData = await this.bookService.createBook(createBookData , file) 
        return responseData 
    }
    @Put("/:bookId") 
    @Roles(Role.EMPLOYEE)
    ...
    {
        const responseData = await this.bookService.updateBookById(Number(bookId) , updateBookData, file) 
        return responseData
    }
    @Delete("/:bookId") 
    @Roles(Role.EMPLOYEE)
